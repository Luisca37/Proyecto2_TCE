...
def modem(Ns, L, Ts, rolloff,hamming_code, iter,total_errores):
    ...
    impulse_modulated_PAM = np.zeros(int(Ns/2 * L))
    for n in range(int(Ns/2)):
        delta_signal = np.concatenate(([array_codificado[n]], np.zeros(L - 1)))
        impulse_modulated_PAM[n * L: (n * L) + L] = delta_signal

    t_tx = np.arange(0, len(impulse_modulated_PAM)) * t_step
    plt.figure(4)
    plt.step(t_tx, impulse_modulated_PAM, where='post')
    plt.xlabel('Tiempo (s)')
    plt.ylabel('Amplitud')
    plt.title('Pulsos')
    plt.grid(True)
