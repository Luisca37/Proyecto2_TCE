...
# Ejemplo de uso
data = [1, 0, 1, 1, 1, 0, 1, 1, 0]  # Datos originales
encoded_data = encode_with_hamming(data)
print("Datos codificados con Hamming Code:", encoded_data)

# Simular un error cambiando un bit al azar
error_position = random.randint(0, len(encoded_data) - 1)
encoded_data[error_position] = 1 - encoded_data[error_position]

decoded_data, error_count = decode_with_hamming(encoded_data)
print("Datos decodificados con corrección de errores:", decoded_data)
print(f"Número de bits con error: {error_count}")
print(f"Número de bits corregidos: {error_count}")